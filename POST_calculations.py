...
def calculate_stuff(vsc_inf, dec, th_titan):
    ...
    vsc_1 = np.sqrt((vsc_inf**2/2) + mu_saturn/r_titan)
    # velocity of spacecraft in cartesian coordinates
    vsc_x = 0
    vsc_y = vsc_1*np.sin(dec*(np.pi/180))
    vsc_z = vsc_1*np.cos(dec*(np.pi/180))
    ...
    v_relx = v_titanx - vsc_x
    v_rely = v_titany - vsc_y
    v_relz = v_titanz - vsc_z
    # convert back to spherical coordinates
    v_mag = np.sqrt(v_relx**2 + v_rely**2 + v_relz**2)
    declination = np.arctan(v_relz/np.sqrt(v_relx**2 + v_rely**2))
